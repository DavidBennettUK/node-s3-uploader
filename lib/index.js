...
Upload = module.exports = function(awsBucketName, opts1) {
  ...
  if ((base3 = this.opts.aws).acl == null) {
    base3.acl = this.opts.awsBucketAcl || 'privat';
  }
...
Image.prototype.resize = function(version, cb) {
  ...
  if (version.crop && (version.crop.x != null) && (version.crop.y != null) && (version.crop.width != null) && (version.crop.height != null)) {
    img = this.gm.crop(version.crop.width, version.crop.height, version.crop.x, version.crop.y).resize(version.maxWidth, version.maxHeight, '!');
  } else {
    img = this.gm.resize(version.maxWidth, version.maxHeight);
  }
  img.quality(version.quality || this.config.opts.resizeQuality);
